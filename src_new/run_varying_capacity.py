...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run WeakTTC baselines with fixed agents/preferences and varying capacity."
    )
    ...
    parser.add_argument("--iterations", type=int, default=10, help="Iterations per capacity.")
...
def main():
    """
    @brief Run experiments varying AgentC capacity.

    Keeps n_e = 50, n_c = 50, and max_rank_size = 2 by default. Sweeps capacity
    over 1, 2, and 5, then writes algorithm metrics to CSV.
    """
    args = parse_args()
    cycle_sort_scheme = "rank_diff_sum"
    preferences_output_path = os.path.join(
        "/tmp", "weakttc_varying_capacity_preferences.json"
    )
    ...
    with open(args.output, "w", newline="", encoding="utf-8") as csv_file:
        ...
        for capacity in [1, 2, 5]:
            rng = random.Random(args.base_seed + capacity)

            for iteration_number in range(1, args.iterations + 1):
                seed = rng.randrange(1_000_000_000)
                print(
                    f"Running n_e={args.n_e}, n_c={args.n_c}, capacity={capacity}, "
                    f"max_rank_size={args.max_rank_size}, iteration={iteration_number}, seed={seed}",
                    flush=True,
                )
                ...
                csv_file.flush()
                print(
                    f"Finished capacity={capacity}, iteration={iteration_number}",
                    flush=True,
                )

    print(f"Results written to {args.output}", flush=True)
    from tools.plot_varying_capacity import plot_results
