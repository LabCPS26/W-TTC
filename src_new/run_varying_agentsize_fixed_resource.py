...
def main():
    """
    @brief Run experiments varying the number of AgentE agents with fixed resources.

    Sweeps n_e from 10 to 100 by 10, keeps n_c fixed at 50, and runs
    ...
    output_path = REPO_ROOT / "results" / "results_varying_agentsize_fixed_resource.csv"
    preferences_output_path = os.path.join(
        "/tmp", "weakttc_varying_agentsize_fixed_resource_preferences.json"
    )
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        for n_e in range(10, 101, 10):
            ...
            for iteration_number in range(1, iterations + 1):
                ...
                print(
                    f"Running n_e={n_e}, n_c={n_c}, iteration={iteration_number}, seed={seed}",
                    flush=True,
                )
    ...
    from tools.plot_varying_agentsize_fixed_resource import plot_results

    plot_paths = plot_results(
        output_path,
        REPO_ROOT / "results" / "agentsize_fixed_resource_plots",
    )
